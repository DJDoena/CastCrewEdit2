...
internal static class DataGridViewHelper
{
    ...
    private static int TryGetBirthYearsInTasks(DataGridView dataGridView
        ...
        , ref int keyIndex)
    {
        ...
        else
        {
            for (var taskIndex = 0; taskIndex < maxTasks; taskIndex++, keyIndex++)
            {
                ...
                var invokeResults = BirthYearGetter.GetBirthYear(persons, defaultValues, log, isCast, addMessage, row);
    ...
    private static int TryGetHeadshotsInTask(DataGridView dataGridView
        , bool useFakeBirthYears
        , bool isCast
        , Action<MessageEntry> addMessage
        , int maxCount
        ...
        , ref int rowIndex)
    {
        ...
        else
        {
            for (var taskIndex = 0; taskIndex < maxTasks; taskIndex++, rowIndex++)
            {
                ...
                if (processedItems.Add(person.PersonLink))
                {
                    HeadshotGetter.Get(useFakeBirthYears, isCast, addMessage, person);
                }
            }
        }
